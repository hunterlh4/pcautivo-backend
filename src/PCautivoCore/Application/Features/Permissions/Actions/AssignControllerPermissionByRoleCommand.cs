...
public class AssignControllerPermissionByRoleCommand : IRequest<Result<IEnumerable<PermissionByRoleDto>>>
{
    public int RoleId { get; set; }
    public string? Controller { get; set; }

    internal sealed class Handler(IRoleRepository roleRepository, IPermissionRepository permissionRepository) : IRequestHandler<AssignControllerPermissionByRoleCommand, Result<IEnumerable<PermissionByRoleDto>>>
    {
        public async Task<Result<IEnumerable<PermissionByRoleDto>>> Handle(AssignControllerPermissionByRoleCommand request, CancellationToken cancellationToken)
        {
            if (request.RoleId <= 0)
            {
            ...
            {
                return Errors.BadRequest("El controlador es requerido.");
            }
            ...
            var filteredPermissions = permissions.Where(p => p.Controller == request.Controller).ToList();
            var rolePermissionIds = rolePermissions.Select(rp => rp.PermissionId).ToHashSet();
            var missingPermissions = filteredPermissions.Where(p => !rolePermissionIds.Contains(p.Id)).ToList();
            ...
            {
                var rolePermissionsToDelete = filteredPermissions.Select(p => new RolePermission
                {
                    RoleId = role.Id,
                    PermissionId = p.Id
                }).ToList();
            ...
            return filteredPermissions.Select(p => new PermissionByRoleDto
            {
                Id = p.Id,
                Name = p.Name,
